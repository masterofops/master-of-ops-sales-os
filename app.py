...
def seed_demo_data():
    ...
    for i, ((comp, contact, title, phone, email), status, step) in enumerate(zip(companies, statuses, steps)):
        ...
        for j in range(random.randint(2, 4)):
            ...
            out = (CALL_OUTCOMES[j % len(CALL_OUTCOMES)] if ch == "Call"
                   else EMAIL_OUTCOMES[j % len(EMAIL_OUTCOMES)] if ch == "Email"
                   else LI_OUTCOMES[j % len(LI_OUTCOMES)])
            acts.append({
...
def tab_setup():
    ...
    steps_data = [
        ("01", "GOOGLE SHEET", "#00A7A7",
         'Go to <a href="https://sheets.google.com" target="_blank" style="color:#00A7A7">sheets.google.com</a>. '
         'Create a blank spreadsheet. Add two tabs named exactly: <code>Leads</code> and <code>ActivityLog</code>.'),
        ...
        ("03", "SHARE SHEET", "#3DBA72",
         "Open your Sheet → Share → paste the service account email → set to <strong>Editor</strong>."),
        ("04", "STREAMLIT SECRETS", "#9B72CF",
         """App Settings → Secrets. Paste and fill in your values:<br>
...
smtp_password = "your-password"</pre>"""),
        ("05", "BOOTSTRAP & DEPLOY", "#00A7A7",
         "Click Bootstrap → Seed Demo → go to the Dial Queue and start executing."),
    ]

    for num, title, color, body in steps_data:
        st.markdown(
            f'<div style="display:flex;gap:1rem;margin:.5rem 0;padding:1rem;background:var(--panel);'
            f'border:1px solid var(--border);border-left:3px solid {color};border-radius:4px;">'
            f'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:1.9rem;font-weight:800;'
            f'color:{color};min-width:38px;line-height:1;">{num}</div>'
            f'<div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:.82rem;font-weight:700;'
            f'letter-spacing:.1em;color:{color};margin-bottom:.3rem;">{title}</div>'
    ...
    with c3:
        ...
        if st.button("🔌 TEST", use_container_width=True, key="btn_test"):
            ...
            if df is not None:
                ...
                st.error("Failed. Check secrets.")

    st.markdown("---")
    st.markdown(sl("SEQUENCE CONFIGURATION (7 STEPS)"), unsafe_allow_html=True)
    seq_html = "".join([
        f'<div style="display:flex;gap:.75rem;padding:.4rem .75rem;border-bottom:1px solid var(--border);'
        f'font-size:.8rem;align-items:center;">'
    ...
    st.markdown(
        ...
        f'<strong style="color:#8899AA;">Extra columns</strong> from CSV/Excel import are preserved automatically.</div>',
        unsafe_allow_html=True,
    )
...
def tab_dial_queue():
    ...
    # ── COCKPIT: Left = action, Right = intel ─────────────────
    ...
    with col_main:
        ...
        st.markdown(
            ...
            f'{channel_badge(channel)}'
            f'<span style="font-family:\'DM Mono\',monospace;font-size:.72rem;color:var(--muted);">'
            f'Step {step_num}/7: {step_info["label"]}</span>'
            f'</div></div>',
            unsafe_allow_html=True,
        )
        ...
        if channel == "Call":
            ...
            st.markdown(
                f'<div class="op-card"><div class="dial-number">{phone or "No phone on file"}</div></div>',
                unsafe_allow_html=True,
            )
            ...
            call_notes   = st.text_area("Call notes *", placeholder="What happened? Intel? Next steps?",
                                        height=80, key="call_notes")
        ...
        if channel == "Email" or st.session_state.email_panel_open:
            ...
            if not identities:
                st.info("No email identities configured. Add them to Streamlit secrets (see Setup tab).")
            else:
                identity_labels = [i["label"] for i in identities]
                sel_label = st.selectbox("Send from", identity_labels, key="email_from")
                sel_ident = next(i for i in identities if i["label"] == sel_label)
                ...
                body    = st.text_area("Body",     value=st.session_state.ai_draft,
                                       height=130, key="email_body")
                    ...
                    if st.button("📤 SEND EMAIL", use_container_width=True, key="btn_send_email"):
                        if not to_addr.strip():
                            ...
                            if ok:
                                ...
                                log_activity(lead_id, str(lead.get("ContactName", "")),
                                             "Email", "Sent",
                                             f"Step {step_num}: {step_info['label']}",
                                             f"Subject: {subject}")
                                st.success(msg)
                                st.session_state.email_panel_open = False
                                st.session_state.active_lead_id   = None
        ...
        with sk2:
            if st.button("🚫 MARK DNC", use_container_width=True, key="btn_dnc"):
                ...
                save_leads(df)
                log_activity(lead_id, str(lead.get("ContactName", "")),
                             "Call", "Not interested", "DNC marked", "")
                st.session_state.active_lead_id = None
                st.rerun()
    ...
    with col_intel:
        ...
        st.markdown(
            ...
            f'<div style="color:var(--muted);font-size:.65rem;text-transform:uppercase;'
            f'letter-spacing:.1em;margin-bottom:.3rem;">Phone</div>'
            f'<div style="font-family:\'DM Mono\',monospace;color:var(--teal);font-size:1rem;'
            f'margin-bottom:.6rem;">{lead.get("Phone", "—")}</div>'
            f'<div style="color:var(--muted);font-size:.65rem;text-transform:uppercase;'
            f'letter-spacing:.1em;margin-bottom:.2rem;">Email</div>'
            f'<div style="font-family:\'DM Mono\',monospace;font-size:.8rem;'
            f'margin-bottom:.6rem;">{lead.get("Email", "—")}</div>'
            f'<div style="color:var(--muted);font-size:.65rem;text-transform:uppercase;'
            f'letter-spacing:.1em;margin-bottom:.2rem;">Company</div>'
        ...
        if notes_val and notes_val != "nan":
            ...
            st.markdown(
                f'<div class="op-card op-card-teal" style="font-size:.82rem;white-space:pre-wrap;'
                f'max-height:180px;overflow-y:auto;">{notes_val}</div>',
                unsafe_allow_html=True,
            )
        ...
        if channel == "LinkedIn":
            ...
            if st.session_state.ai_draft:
                st.markdown(
                    f'<div class="op-card" style="border-color:var(--purple);font-size:.82rem;'
                    f'white-space:pre-wrap;">{st.session_state.ai_draft}</div>',
                    unsafe_allow_html=True,
                )
...
def tab_analytics():
    ...
    cutoff = now - timedelta(days={"Last 7 days": 7, "Last 14 days": 14,
                                    "Last 30 days": 30, "All time": 3650}[rng])
    ...
    def ch_out(ch, out_filter=None):
        ...
        if out_filter:
            sub = sub[sub["Outcome"].isin(out_filter)]
        return len(sub)
    ...
    with col_lb:
        ...
        if not act.empty:
            ...
            for rank, (_, r) in enumerate(lb.iterrows(), 1):
                ...
                lb_html += (
                    f'<div class="lb-row">'
                    f'<div class="lb-rank {rc}">#{rank}</div>'
                    f'<div class="lb-name">{r["Username"]}</div>'
                    f'<div class="lb-bar-wrap"><div class="lb-bar" style="width:{pct}%"></div></div>'
                    f'<div class="lb-count">{r["touches"]}</div>'
                    f'</div>'
                )
        ...
        if not act.empty and "Channel" in act.columns:
            ...
            pivot = (
                act.groupby(["Username", "Channel"]).size()
                .reset_index(name="n")
        ...
        if not leads_df.empty:
            ...
            for s in SEQUENCE:
                ...
                fn_html += (
                    ...
                    f'<div class="funnel-label">Step {s["step"]}: {s["label"]}</div>'
                    f'<div class="funnel-bar-wrap"><div class="funnel-bar" style="width:{p}%;background:{colors[s["channel"]]}"></div></div>'
                    f'<div class="funnel-pct">{p}%</div>'
                    f'<div class="funnel-n">{n}</div>'
                    f'</div>'
                )
            for label, filt, color in [
                ("Warm 🌡", leads_df["Status"] == "Warm",  "#F0A500"),
                ("Hot 🔥",  leads_df["Status"] == "Hot",   "#E05252"),
                ("DNC 🚫",  leads_df["Status"] == "DNC",   "#4A5A6A"),
            ]:
                n = int(filt.sum())
                p = round(n / total_a * 100)
                fn_html += (
                    ...
                    f'<div class="funnel-bar-wrap"><div class="funnel-bar" style="width:{p}%;background:{color}"></div></div>'
                    f'<div class="funnel-pct">{p}%</div>'
                    f'<div class="funnel-n">{n}</div>'
                    f'</div>'
                )
    ...
    if not act.empty and "Timestamp" in act.columns:
        ...
        bars   = "".join([
            f'<div title="{r["date"]}: {r["count"]}" '
            f'style="flex:1;min-width:4px;height:{max(int(r["count"]/max_c*68),2)}px;'
            f'background:#00A7A7;border-radius:2px 2px 0 0;opacity:{0.3+0.7*(r["count"]/max_c):.2f}"></div>'
            for _, r in daily.iterrows()
        ])
        st.markdown(
            f'<div class="op-card">'
            f'<div style="display:flex;align-items:flex-end;gap:2px;height:72px;">{bars}</div>'
            ...
            f'<span style="font-size:.62rem;color:var(--muted);">{daily["date"].iloc[-1]}</span>'
            f'</div></div>',
            unsafe_allow_html=True,
        )
    ...
    if not act.empty:
        ...
        for _, r in feed.iterrows():
            ...
            feed_html += (
                ...
                f'<span style="color:var(--muted);font-family:\'DM Mono\',monospace;font-size:.67rem;min-width:52px;">{age}</span>'
                f'<span style="font-weight:600;min-width:65px;color:var(--teal)">{r.get("Username","")}</span>'
                f'<span style="color:{cc};min-width:62px;font-size:.75rem;">{ch}</span>'
                f'<span style="color:#CCDDEE;min-width:80px;">{r.get("ContactName","")}</span>'
                f'<span style="color:var(--muted);font-style:italic;font-size:.78rem;">{r.get("Outcome","")}</span>'
                f'</div>'
            )
        st.markdown(f'<div class="op-card" style="max-height:280px;overflow-y:auto;">{feed_html}</div>',
                    unsafe_allow_html=True)
...
def tab_manage_leads():
    ...
    with st.expander("📥 IMPORT FROM CSV / EXCEL", expanded=False):
        ...
        if uploaded is not None:
            try:
                ...
                with ic2:
                    default_step = st.selectbox(
                        "Start at sequence step", list(range(1, 8)),
                        format_func=lambda x: f"Step {x}: {get_step_info(x)['label']}",
                    )
                    mapped["SequenceStep"] = mapped["SequenceStep"].where(
                ...
                if st.button("✅ CONFIRM IMPORT", use_container_width=True, key="btn_confirm_import"):
                    if append_mode == "Append to existing" and not df.empty:
                        ...
                        mp_full  = mapped.reindex(columns=all_cols)
                        combined = pd.concat([df_full, mp_full], ignore_index=True)
    ...
    with st.expander("➕ ADD SINGLE LEAD", expanded=False):
        ...
        with c2:
            ...
            ind   = st.text_input("Industry", key="new_ind")
        with c3:
            rev    = st.text_input("Revenue", key="new_rev")
            status = st.selectbox("Status", STATUSES, key="new_status")
            step   = st.selectbox(
                "Start Step", list(range(1, 8)),
                format_func=lambda x: f"Step {x}: {get_step_info(x)['label']}",
                key="new_step",
            )
    ...
    with f1:
        fst = st.multiselect("Status", STATUSES, default=STATUSES)
    with f2:
        fsp = st.multiselect("Step", list(range(1, 8)), default=list(range(1, 8)),
                              format_func=lambda x: f"S{x}: {get_step_info(x)['label'][:12]}")
    with f3:
        fsq = st.text_input("Search", placeholder="Name / Company…", key="search_leads")

    if not df.empty:
        v = df.copy()
        if fst:
            v = v[v["Status"].isin(fst)]
        if fsp:
            v = v[v["SequenceStep"].isin(fsp)]
        if fsq.strip():
            s = fsq.lower()
            v = v[
                v["ContactName"].str.lower().str.contains(s, na=False) |
                v["Company"].str.lower().str.contains(s, na=False)
            ]
    ...
    if not df.empty:
        opts   = {f"{r['ContactName']} — {r['Company']} ({r['LeadID']})": r["LeadID"]
                  for _, r in df.iterrows()}
        chosen = st.selectbox("Select Lead", ["— select —"] + list(opts.keys()), key="edit_lead_sel")
        if chosen != "— select —":
            row  = df[df["LeadID"] == opts[chosen]].iloc[0]
            ec1, ec2 = st.columns(2)
            with ec1:
                es = st.selectbox("Status", STATUSES,
                                  index=STATUSES.index(str(row.get("Status", "Cold"))), key="edit_status")
                ep = st.selectbox("Step", list(range(1, 8)),
                                  index=max(int(row.get("SequenceStep", 1)) - 1, 0),
                                  format_func=lambda x: f"{x}: {get_step_info(x)['label']}", key="edit_step")
            with ec2:
            ...
            if st.button("💾 UPDATE LEAD", use_container_width=True, key="btn_update_lead"):
                ...
                if save_leads(df):
                    log_activity(opts[chosen], str(row.get("ContactName", "")),
                                 "", "", f"Manual Update → {es}", en)
                    st.success("Updated ✓")
                    st.rerun()
...
# ─────────────────────────────────────────────
def tab_activity():
    st.markdown('<h2 style="color:#00A7A7;font-size:1.4rem;">📋 TEAM ACTIVITY LOG</h2>',
                unsafe_allow_html=True)
    ...
    with k4:
        dials_today = len(tdf[tdf["Channel"] == "Call"]) if "Channel" in tdf.columns else 0
        st.metric("Dials Today", dials_today)
    with k5:
        emails_today = len(tdf[tdf["Channel"] == "Email"]) if "Channel" in tdf.columns else 0
        st.metric("Emails Today", emails_today)

    st.markdown("---")
    f1, f2, f3 = st.columns(3)
    with f1:
        fu = st.selectbox("Operator", ["All"] + sorted(df["Username"].dropna().unique().tolist()),
                          key="act_filter_user")
    with f2:
    ...
    if fo.strip() and "Outcome" in show.columns:
        s    = fo.lower()
        show = show[
            show["Outcome"].str.lower().str.contains(s, na=False) |
            show["Notes"].str.lower().str.contains(s, na=False)
        ]
...
# ─────────────────────────────────────────────
def tab_ai():
    st.markdown('<h2 style="color:#00A7A7;font-size:1.4rem;">🤖 AI MESSAGE WORKSHOP</h2>',
                unsafe_allow_html=True)
    ...
    with c2:
        ...
        mtype    = st.radio("Type", ["Follow-up Email", "LinkedIn Message"],
    ...
    if st.button("⚡ GENERATE DRAFT", use_container_width=True, key="btn_ai_generate"):
        if not contact or not company:
            ...
            row = {"ContactName": contact, "Company": company,
                   "Industry": industry, "Revenue": revenue, "Notes": notes}
...
def login_screen():
    st.markdown(
        ...
        'letter-spacing:.15em;color:#00A7A7;text-transform:uppercase;">⚙ Master of Ops</div>'
        '<div style="font-size:.68rem;letter-spacing:.3em;color:#8899AA;text-transform:uppercase;'
        'margin-top:-.7rem;">Team Execution OS v3 — Auto-Dial Outbound Cockpit</div>'
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            ...
            f'<div class="logo-mark">⚙ Master of Ops</div>'
            f'<div class="logo-sub">Execution OS v3.0</div></div>'
            f'<div style="font-size:.68rem;color:#8899AA;margin-bottom:.4rem;">'
            f'Operator: <span style="color:#00A7A7;font-weight:600">{st.session_state.username}</span></div>'
        ...
        if st.button("⬅ Logout", use_container_width=True, key="btn_logout"):
            ...
            for k in ["username", "active_lead_id", "ai_draft", "ai_subject"]:
                st.session_state[k] = ""
            st.rerun()
        ...
        for line in [
            ...
            st.markdown(f'<div style="font-size:.76rem;color:#8899AA;padding:.18rem 0;">{line}</div>',
                        unsafe_allow_html=True)

        st.markdown("---")
        st.markdown(
            '<div style="font-size:.58rem;color:#4A5A6A;line-height:1.7;text-align:center;">'
            '7-step sequence | Locks: 10 min<br>'
            'SMTP email | tel: dialer links<br>'
            'CSV/Excel import | URL columns</div>',
            unsafe_allow_html=True,
        )
